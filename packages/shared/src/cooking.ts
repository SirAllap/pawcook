...
const METHOD_INSTRUCTIONS = {
  ...
  raw: 'Serve meat raw — no cooking required. Ensure meat is fresh and appropriate for raw feeding. For bone content, always use raw meaty bones. Keep cold and serve within safe food handling guidelines.',
} as const;

// Base cook times (minutes) at ~1 kg for each form × method
const FORM_BASE_TIMES: Record<string, Record<string, number>> = {
  minced:    { oven: 20,  stovetop_low: 8,  slow_cooker: 150, pressure_cooker: 10, raw: 0 },
  cubed:     { oven: 40,  stovetop_low: 18, slow_cooker: 240, pressure_cooker: 18, raw: 0 },
  whole_cut: { oven: 65,  stovetop_low: 35, slow_cooker: 330, pressure_cooker: 35, raw: 0 },
  fillet:    { oven: 30,  stovetop_low: 12, slow_cooker: 180, pressure_cooker: 8, raw: 0 },
};
...
function calcBatch(
  method: 'oven' | 'stovetop_low' | 'slow_cooker' | 'pressure_cooker' | 'raw',
  input: CookingInput,
